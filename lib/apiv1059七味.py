...
class Spider(Spider):
    ...
    def homeVideoContent(self):
        ...
        try:
            ...
            for item in html.xpath('//ul[contains(@class, "content-list")]/li')[:30]:
                a_tag = item.xpath('./qyg3.js')[0]
                href = a_tag.get('href', '')
                title = a_tag.get('title', '')
                if title and href:
                    img = str(a_tag.xpath('./qyg4.js')[0]) if a_tag.xpath('./qyg4.js') else ''
                    desc = str(item.xpath('./qyg6.js')[0]).strip() if item.xpath('./qyg6.js') else ''
    ...
    def detailContent(self, ids):
        ...
        try:
            ...
            h1_text = ''.join(map(str, root.xpath('./qyg13.js'))).strip()
            ...
            VOD['vod_pic'] = str(root.xpath('./qyg14.js')[0]) if root.xpath('./qyg14.js') else ''
            for div in root.xpath('./qyg16.js'):
                text = ''.join(map(str, div.xpath('./qyg17.js'))).strip()
                a_text = '/'.join(map(str, div.xpath('./qyg18.js')))
            ...
            VOD['vod_remarks'] = ''.join(map(str, root.xpath('./qyg19.js'))).strip()
            VOD['vod_content'] = ''.join(map(str, root.xpath('./qyg20.js'))).strip()
            play_from, play_url = [], []
            ...
            for i, (line_item, container) in enumerate(zip(line_items, episode_containers)):
                line_name = re.sub(r'\d+', '', ''.join(map(str, line_item.xpath('./qyg17.js'))).strip().split('\n')[0]).strip().replace(' ', '')
                if not line_name:
                    continue
                ...
                for j, ep_item in enumerate(container.xpath('.//a')):
                    try:
                        ep_title = ''.join(map(str, ep_item.xpath('./qyg17.js'))).strip()
        ...
        try:
            ...
            for item in home_data:
                try:
                    ...
                    img = str(a_tag.xpath('./qyg4.js')[0]) if a_tag.xpath('./qyg4.js') else ''
                    desc = item.xpath('./qyg6.js')[0].strip() if item.xpath('./qyg6.js') else ''
                    vod_id = re.search(r'/mv/(\d+)\.html', href).group(1) if re.search(r'/mv/(\d+)\.html', href) else ''
                    results.append({
                        "vod_id": vod_id,
                        "vod_name": title,
